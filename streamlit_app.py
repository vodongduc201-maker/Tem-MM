            ...
            if st.button("🚀 XUAT FILE PDF"):
                ...
                for _, row in df_gop.iterrows():
                    ...
                    for i in range(1, tk + 1):
                        ...
                        c.drawString(0.4*cm, 0.6*cm, str(row.get(c_ma, '')))
                        c.drawRightString(width-0.4*cm, 0.6*cm, str(row.get(c_ten, '')))

                        c.showPage()

                c.save()
                st.download_button("📥 TAI FILE PDF VE", buffer.getvalue(), "Tem_Giao_Hang.pdf")
